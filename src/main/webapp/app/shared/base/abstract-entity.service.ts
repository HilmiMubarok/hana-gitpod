...
export class AbstractEntityService<T> {
  ...
  create(entity: T, params?: any): Observable<HttpResponse<T>> {
    ...
    return this.http
      .post<T>(this.resourceUrl, entity, { observe: 'response', params: options })
  ...
  saveAll(entities: T[], params?: any): Observable<HttpResponse<T[]>> {
    ...
    const options = createRequestOption(params);
    return this.http
      .post<T[]>(`${this.resourceUrl}/save-all`, entities, { observe: 'response', params: options })
  ...
  update(entity: T, params?: any): Observable<HttpResponse<T>> {
    ...
    const options = createRequestOption(params);
    return this.http
      .put<T>(this.resourceUrl, entity, { observe: 'response', params: options })
      .pipe(map((res: HttpResponse<T>) => this.convertDateFromServer(res)))
      .pipe(map((res: HttpResponse<T>) => this.preLoadItem(res)));
  }
  ...
  loadCacheAll(): Observable<T[]> {
    ...
    if ((this.caches$ && minutes < this.cacheTTL) || (this.cacheTTL === 0 && this.cacheSubject.getValue().length > 0)) {
  ...
  getTasks(id: any): Observable<HttpResponse<IProcessTask[]>> {
    return this.http.get<IProcessTask[]>(`${this.resourceUrl}/process-task/${id}`, { observe: 'response' });
  }
