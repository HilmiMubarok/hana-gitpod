...
export class DppkFinalizeService extends AbstractEntityService<IDppkFinalize> {
  ...
  private changgedColRelByCP?: IDppkFinalize;
  private triggerChanggedColRelByCP = new BehaviorSubject<IDppkFinalize>(this.changgedColRelByCP);
  public triggerChanggedColRelByCPObservable = this.triggerChanggedColRelByCP.asObservable();

  public changeColRelByCP(newCP: IDppkFinalize) {
  ...
  public applicationGroubProduct(id: number): Observable<HttpResponse<any>> {
    return this.http
      .get<IDppkFinalize[]>(MICROSERVICENAME.LOS + '/api/application-group-products/page/' + id, { observe: 'response' })
  ...
  protected convertDateArrayFromServer(res: HttpResponse<IDppkFinalize[]>): HttpResponse<IDppkFinalize[]> {
    res.body.forEach((creditProposal: IDppkFinalize) => {
      //
