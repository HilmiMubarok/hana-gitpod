...
import { DppkFinalize, IDppkFinalize } from './dppk-finalize.model';
...
export class DppkFinalizeReviewResolve implements Resolve<IDppkFinalize> {
  constructor(private service: DppkFinalizeService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IDppkFinalize> | Observable<never> {
    const useTemplate = 'PERSON';
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((creditProposal: HttpResponse<IDppkFinalize>) => {
    ...
    if (useTemplate) {
      return this.service.template(useTemplate).pipe(
        map((res: HttpResponse<IDppkFinalize>) => res.body),
        mergeMap(res => {
          if (res) {
            return of(res);
          } else {
    ...
    const newItem = new DppkFinalize();
...
export const DppkFinalizeReviewRoute: Routes = [
  {
    path: '',
    component: DppkFinalizeComponent,
    resolve: {
  ...
  {
    path: ':id/edit',
    component: DppkFinalizeDetailComponent,
    resolve: {
      content: DppkFinalizeReviewResolve,
    },
    data: {
      authorities: ['ROLE_USER'],
      pageTitle: 'losgwApp.partyCif.home.title',
