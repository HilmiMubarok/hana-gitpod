...
@Component({
  selector: 'jhi-organization-legal-view',
  templateUrl: './organization-legal-view.component.html',
})
export class OrganizationLegalViewComponent extends AbstractEntityBaseViewComponent<IOrganizationLegal> implements OnChanges {
  @Input() id: number;
  ...
  constructor(
    ...
    super(organizationLegalService, messageService, elementRef, dataUtils, account, eventManager);
    this.item = new OrganizationLegal();
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['id']) {
      ...
      if (this.id) {
        this.item = new OrganizationLegal();
        this.organizationLegalService.find(this.id).subscribe(result => {
  ...
  set organizationLegal(organizationLegal: IOrganizationLegal) {
    this.item = organizationLegal;
  }
