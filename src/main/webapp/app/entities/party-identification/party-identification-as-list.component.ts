...
@Component({
  selector: 'jhi-party-identification-as-list',
  templateUrl: './party-identification-as-list.component.html',
})
export class PartyIdentificationAsListComponent extends AbstractEntityAsListComponent<IPartyIdentification> implements OnChanges {
  @Input() filterName: string;
  @Input() idIdentificationType: any;
  @Input() idParty: any;
  ...
  protected loadAllFilterBy() {
    ...
    if (this.idIdentificationType) {
      queryParams.idIdentificationType = this.idIdentificationType;
    }
    if (this.idParty) {
      queryParams.idParty = this.idParty;
    }
    this.partyIdentificationService.queryFilterBy(queryParams).subscribe(
      (res: HttpResponse<IPartyIdentification[]>) => this.paginateItems(res.body, res.headers),
      (res: HttpErrorResponse) => this.onError(res.message)
    );
  }

  ngOnChanges(changes: SimpleChanges) {
    if (changes['idIdentificationType']) {
      this.loadAll();
    }
    if (changes['idParty']) {
      this.loadAll();
  ...
  trackId(index: number, item: IPartyIdentification) {
    return item.id;
  }

  get partyIdentifications() {
    return this.items;
  }

  set partyIdentifications(partyIdentification: IPartyIdentification[]) {
    this.items = partyIdentification;
  }
  ...
  addNewData() {
    ...
    if (this.idIdentificationType) {
      queryParams.identificationTypeId = this.idIdentificationType;
    }
    if (this.idParty) {
      queryParams.partyId = this.idParty;
    }
    this.router.navigate(['party-identification/new', queryParams]);
  }

  onEditComplete(event: any) {
    this.partyIdentificationService.update(event.data).subscribe(() => {
