...
@Component({
  ...
  templateUrl: './credit-agreement.component.html',
...
export class CreditAgreementComponent extends AbstractEntityMaterialComponent<ICreditAgreement> implements OnInit {
  ...
  constructor(
    ...
    private cashCreditAgreementService: CashCreditAgreementService,
    private templateService: TemplateService
  ) {
    super(_snackBar, creditAgreementService);
    this.page = 0;
  ...
  public doSearch(): void {
    ...
    predicate['target'] = 'finalize-credit-agreement';

    this.cashCreditAgreementService
      .searchCP(predicate)
      .pipe(map((res: HttpResponse<ICreditAgreement[]>) => this.preLoad(res)))
      .subscribe({
        next: (res: HttpResponse<ICreditAgreement[]>) => {
  ...
  private checkReturnStatusDescription(data: ICreditAgreement[]) {
    if (data.length > 0) {
  ...
  public showTimeLine(element: ICreditAgreement): void {
