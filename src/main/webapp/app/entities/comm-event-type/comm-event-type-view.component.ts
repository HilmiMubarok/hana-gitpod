...
@Component({
  selector: 'jhi-comm-event-type-view',
  templateUrl: './comm-event-type-view.component.html',
})
export class CommEventTypeViewComponent extends AbstractEntityBaseViewComponent<ICommEventType> implements OnChanges {
  @Input() id: string;
  ...
  constructor(
    ...
    super(commEventTypeService, messageService, elementRef, dataUtils, account, eventManager);
    this.item = new CommEventType();
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['id']) {
      ...
      if (this.id) {
        this.item = new CommEventType();
        this.commEventTypeService.find(this.id).subscribe(result => {
  ...
  set commEventType(commEventType: ICommEventType) {
    this.item = commEventType;
  }
