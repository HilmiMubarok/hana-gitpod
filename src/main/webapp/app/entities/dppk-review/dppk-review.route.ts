...
import { DppkReview, IDppkReview } from './dppk-review.model';
...
export class DppkReviewResolve implements Resolve<IDppkReview> {
  constructor(private service: DppkReviewService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IDppkReview> | Observable<never> {
    const useTemplate = 'PERSON';
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((creditProposal: HttpResponse<IDppkReview>) => {
    ...
    if (useTemplate) {
      return this.service.template(useTemplate).pipe(
        map((res: HttpResponse<IDppkReview>) => res.body),
        mergeMap(res => {
          if (res) {
            return of(res);
          } else {
...
export const DppkReviewRoute: Routes = [
  {
    path: '',
    component: DppkReviewComponent,
    resolve: {
  ...
  {
    path: ':id/edit',
    component: DppkReviewDetailComponent,
    resolve: {
      content: DppkReviewResolve,
    },
    data: {
      authorities: ['ROLE_USER'],
      pageTitle: 'losgwApp.partyCif.home.title',
