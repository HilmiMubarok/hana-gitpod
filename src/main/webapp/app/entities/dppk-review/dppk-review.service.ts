...
export class DppkReviewService extends AbstractEntityService<IDppkReview> {
  ...
  private changgedColRelByCP?: IDppkReview;
  private triggerChanggedColRelByCP = new BehaviorSubject<IDppkReview>(this.changgedColRelByCP);
  public triggerChanggedColRelByCPObservable = this.triggerChanggedColRelByCP.asObservable();

  public changeColRelByCP(newCP: IDppkReview) {
  ...
  public applicationGroubProduct(id: number): Observable<HttpResponse<any>> {
    return this.http
      .get<IDppkReview[]>(MICROSERVICENAME.LOS + '/api/application-group-products/page/' + id, { observe: 'response' })
  ...
  protected convertDateArrayFromServer(res: HttpResponse<IDppkReview[]>): HttpResponse<IDppkReview[]> {
    res.body.forEach((creditProposal: IDppkReview) => {
      //
