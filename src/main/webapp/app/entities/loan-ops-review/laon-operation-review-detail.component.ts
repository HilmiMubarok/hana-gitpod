...
export class LoanOpsReviewDetailComponent implements OnInit {
  ...
  public creditProposal: ILoanOPSReview;
  public creditProposalStartState: ILoanOPSReview;
  ...
  constructor(
    ...
    private loanOpsReviewProcessService: LoanOpsReviewProcessService,
  ...
  public conditionSaveBtn() {
    if (this.router.url.includes('loan-ops-review')) {
      ...
      this.loanOpsReviewService.update(this.preSave(statusPreSave)).subscribe(res => {
        ...
        if (this.saveState === 'process') {
          if (this.parentPath === 'loan-ops-review') {
            ...
            this.loanOpsReviewProcessService.processTask(this.resAttr).subscribe(() => {
  ...
  ngOnInit() {
    ...
    this.loanOpsReviewService.find(this.activatedRoute.snapshot.data['content'].id).subscribe((response: any) => {
      const menuItemIdByRoute = this.router.url.includes('loan-ops-review') ? 'LOAN_OPERATION_REVIEW' : 'LOAN_OPERATION_REVIEW';
  ...
  private preSave(status: string): ILoanOPSReview {
    ...
    const copyCreditProposal: ILoanOPSReview = lodash.cloneDeep(this.creditProposal);
    ...
    copyCreditProposal.attributes['disbursementPlan'] = JSON.stringify(copyCreditProposal.attributes['disbursementPlan']);
