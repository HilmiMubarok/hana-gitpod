...
import { ILoanOPSReview, LoanOpsReviewModel } from './laon-operation-review.model';
...
export class LoanOpsReviewResolve implements Resolve<ILoanOPSReview> {
  constructor(private service: LoanOpsReviewService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<ILoanOPSReview> | Observable<never> {
    const useTemplate = 'PERSON';
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((creditProposal: HttpResponse<ILoanOPSReview>) => {
    ...
    if (useTemplate) {
      return this.service.template(useTemplate).pipe(
        map((res: HttpResponse<ILoanOPSReview>) => res.body),
        mergeMap(res => {
          if (res) {
            return of(res);
          } else {
    ...
    const newItem = new LoanOpsReviewModel();
...
export const LoanOpsReviewRoute: Routes = [
  {
    path: '',
    component: LoanOpsReviewComponent,
    resolve: {
  ...
  {
    path: ':id/edit',
    component: LoanOpsReviewDetailComponent,
    resolve: {
      content: LoanOpsReviewResolve,
    },
    data: {
      authorities: ['ROLE_USER'],
      pageTitle: 'losgwApp.partyCif.home.title',
