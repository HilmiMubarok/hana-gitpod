...
@Component({
  selector: 'jhi-button-generate-dar',
  templateUrl: './loan-analys-generate-dar.component.html',
})
export class LoanAnalysGenerateDarComponent implements OnInit {
  ...
  private generateFileDar(fileType: string, api: string, req?: any) {
    ...
    this.http.get(api, { params: options, responseType: 'text', observe: 'response' }).subscribe(response => {
      let fileName;
      if ((fileName = fileType === 'Word')) {
        response.body.slice(-34);
      }
      this.messageService.add({
        severity: 'success',
        summary: 'Success',
