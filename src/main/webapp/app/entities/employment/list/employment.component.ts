...
export class EmploymentComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.employmentService
        ...
        .subscribe({
          next: (res: HttpResponse<IEmployment[]>) => {
  ...
  search(query: string): void {
    if (
      query &&
      ['relationTypeId', 'relationTypeDescription', 'partyToId', 'partyToName', 'partyFromId', 'partyFromName'].includes(this.predicate)
    ) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  trackId(_index: number, item: IEmployment): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IEmployment[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
