...
@Component({
  selector: 'jhi-status-item',
  templateUrl: './status-item.component.html',
})
export class StatusItemComponent extends AbstractEntityComponent<IStatusItem> {
  constructor(
  ...
  trackId(index: number, item: IStatusItem) {
    return item.id;
  }
  ...
  set statusItems(statusItem: IStatusItem[]) {
    this.items = statusItem;
  }
}
