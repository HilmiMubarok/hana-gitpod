...
import { ILoanOPS, LoanOperation } from './loan-operation.model';
...
export class LoanOperationResolve implements Resolve<ILoanOPS> {
  constructor(private service: LoanOperationService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<ILoanOPS> | Observable<never> {
    const useTemplate = 'PERSON';
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((creditProposal: HttpResponse<ILoanOPS>) => {
    ...
    if (useTemplate) {
      return this.service.template(useTemplate).pipe(
        map((res: HttpResponse<ILoanOPS>) => res.body),
        mergeMap(res => {
          if (res) {
            return of(res);
          } else {
    ...
    const newItem = new LoanOperation();
...
export const LoanOperationRoute: Routes = [
  {
    path: '',
    component: LoanOperationComponent,
    resolve: {
  ...
  {
    path: ':id/edit',
    component: LoanOperationDetailComponent,
    resolve: {
      content: LoanOperationResolve,
    },
    data: {
      authorities: ['ROLE_USER'],
      pageTitle: 'losgwApp.partyCif.home.title',
