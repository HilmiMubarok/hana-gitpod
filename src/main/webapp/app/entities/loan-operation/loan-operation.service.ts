...
export class LoanOperationService extends AbstractEntityService<ILoanOPS> {
  ...
  private changgedColRelByCP?: ILoanOPS;
  private triggerChanggedColRelByCP = new BehaviorSubject<ILoanOPS>(this.changgedColRelByCP);
  public triggerChanggedColRelByCPObservable = this.triggerChanggedColRelByCP.asObservable();

  public changeColRelByCP(newCP: ILoanOPS) {
  ...
  public applicationGroubProduct(id: number): Observable<HttpResponse<any>> {
    return this.http
      .get<ILoanOPS[]>(MICROSERVICENAME.LOS + '/api/application-group-products/page/' + id, { observe: 'response' })
  ...
  protected convertDateArrayFromServer(res: HttpResponse<ILoanOPS[]>): HttpResponse<ILoanOPS[]> {
    res.body.forEach((creditProposal: ILoanOPS) => {
      //
