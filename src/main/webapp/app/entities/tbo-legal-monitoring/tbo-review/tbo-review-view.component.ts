...
import { ITboReviewModel } from './tbo-review.model';
...
import { LendingProgramParameterService } from 'app/entities/lending-program-parameter/lending-program-parameter.service';
import { MessageService } from 'primeng/api';
import { formatBytes } from 'app/shared/helper/utils';
import { TboLegalMonitoringService } from '../tbo-legal-monitoring-process.service';
import { IProcessTask } from 'app/shared/model/process-task.model';
import { TaskCommentDialogComponent } from 'app/layouts/miscellaneous/task-comment-dialog.component';
import { TboReviewService } from './tbo-review.service';
import { CreditProposalTabBusinessActivityComponent } from 'app/entities/credit-proposal/busines-activity/credit-proposal-tab-business-activity.component';
import { CPMemoBandingRemarkComponent } from 'app/entities/credit-proposal/memo-banding/remarks/cp-memo-banding-remark.component';
...
import { CreditProposalTabLoanFacilityDetailComponent } from 'app/entities/credit-proposal/loan-facility/credit-proposal-tab-loan-facility-detail.component';
...
@Component({
  ...
  styleUrls: ['./tbo-review.style.css'],
})
export class TboReviewViewComponent implements OnInit {
  ...
  public creditProposal: ITboReviewModel;
  public creditProposalStartState: ITboReviewModel;
  ...
  public headerTitle = 'Basic Information ';
  ...
  constructor(
    ...
    private tboLegalMonitoringService: TboLegalMonitoringService,
  ...
  public conditionSaveBtn() {
    if (this.router.url.includes('tbo-legal-review')) {
  ...
  setIsAllowSave(status: boolean) {
    ...
    if (this.creditProposal.id) {
      this.tboReviewService.update(this.preSave(statusPreSave)).subscribe(res => {
        ...
        if (this.saveState === 'process') {
          if (this.parentPath === 'tbo-legal-review') {
            ...
            this.tboLegalMonitoringService.processTask(this.resAttr).subscribe(() => {
  ...
  private saveUpdate(status: string, source: string): void {
    this.tboReviewService.update(this.preSave(status)).subscribe(res => {
  ...
  public save(source: string): void {
    ...
    if (this.creditProposal.attributes.proposalType === null || this.creditProposal.attributes.proposalType === '') {
      ...
      if (this.creditProposal.id) {
        // if (this.router.url.split('/')[1] === 'finalize-dpdl') {
        //   this.saveUpdate('not-complete', source);
        // } else
        if (this.router.url.split('/')[1] === 'tbo-legal-review') {
          if (this.creditProposalOpinionHistoryComponent) {
        ...
        /* this.dpdlFinalizeService.create(this.preSave()).subscribe(res => {
          this.creditProposal.collaterals = res.body.collaterals;
          this.creditProposal.products = res.body.products;
          if (this.creditProposalTabBusinessActivityComponent) {
            this.creditProposalTabBusinessActivityComponent.triggeredSaveAll();
  ...
  private preSave(status: string): ITboReviewModel {
    for (let i = 0; i < this.tboReviewService.partySliks.length; i++) {
      this.creditProposal.sliks = [...this.creditProposal.sliks, this.tboReviewService.partySliks[i]];
    }
    const copyCreditProposal: ITboReviewModel = lodash.cloneDeep(this.creditProposal);

    if (this.router.url.split('/')[1] === 'dar-revision') {
