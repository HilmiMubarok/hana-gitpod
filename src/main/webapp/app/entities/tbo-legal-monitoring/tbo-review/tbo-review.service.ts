...
export class TboReviewService extends AbstractEntityService<ITboReviewModel> {
  ...
  private changgedColRelByCP?: ITboReviewModel;
  private triggerChanggedColRelByCP = new BehaviorSubject<ITboReviewModel>(this.changgedColRelByCP);
  public triggerChanggedColRelByCPObservable = this.triggerChanggedColRelByCP.asObservable();

  public changeColRelByCP(newCP: ITboReviewModel) {
    this.changgedColRelByCP = newCP;
    this.triggerChanggedColRelByCP.next(this.changgedColRelByCP);
  }

  protected isNew(entity: ITboReviewModel): boolean {
  ...
  public applicationGroubProduct(id: number): Observable<HttpResponse<any>> {
    return this.http
      .get<ITboReviewModel[]>(MICROSERVICENAME.LOS + '/api/application-group-products/page/' + id, { observe: 'response' })
      .pipe(map((res: HttpResponse<any[]>) => this.convertDateArrayFromServer(res)))
      .pipe(map((res: HttpResponse<any[]>) => this.preLoadItemArray(res)));
  }

  protected convertDateArrayFromServer(res: HttpResponse<ITboReviewModel[]>): HttpResponse<ITboReviewModel[]> {
    res.body.forEach((creditProposal: ITboReviewModel) => {
      //
  ...
  protected preSave(entity: ITboReviewModel) {
  ...
  public findByCif(cif: string): Observable<HttpResponse<ITboReviewModel>> {
    return this.http.get<ITboReviewModel>(this.resourceUrl + '/cif/' + cif, { observe: 'response' });
  }

  public findPersonTemplate(cif: string): Observable<HttpResponse<ITboReviewModel>> {
    return this.http.get<ITboReviewModel>(this.resourceUrl + '/cif-person-template/' + cif, { observe: 'response' });
  }

  public findPartyGroupTemplate(cif: string): Observable<HttpResponse<ITboReviewModel>> {
    return this.http.get<ITboReviewModel>(this.resourceUrl + '/cif-organization-template/' + cif, { observe: 'response' });
  }
  ...
  public getListRetrive(cif?: string, req?: any): Observable<HttpResponse<any>> {
    ...
    return this.http
      .get<ITboReviewModel[]>(url, { params: options, observe: 'response' })
      .pipe(map((res: HttpResponse<ITboReviewModel[]>) => this.convertDateArrayFromServer(res)))
      .pipe(map((res: HttpResponse<ITboReviewModel[]>) => this.preLoadItemArray(res)));
  }
