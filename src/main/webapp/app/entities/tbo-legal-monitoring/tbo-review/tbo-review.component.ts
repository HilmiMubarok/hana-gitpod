...
@Component({
  ...
  templateUrl: './tbo-review.component.html',
...
export class TboReviewComponent extends AbstractEntityMaterialComponent<ITboReviewModel> implements OnInit {
  ...
  constructor(
    ...
    super(_snackBar, tboReviewService);
    this.page = 0;
  ...
  public generateDocumentTBO() {
    ...
    this.tboReviewService.generateDocument().subscribe({
  ...
  public doSearch(): void {
    ...
    this.cashTboLegalMonitoringService
      ...
      .subscribe({
        next: (res: HttpResponse<ITboReviewModel[]>) => {
  ...
  private checkReturnStatusDescription(data: ITboReviewModel[]) {
    if (data.length > 0) {
      ...
      if (this.router.url === '/tbo-legal-review') {
        ...
        if (this.clickedChip['statusId'] !== '') {
          this.cashTboLegalMonitoringService
            ...
            .pipe(map((res: HttpResponse<ITboReviewModel[]>) => this.preLoad(res)))
            .subscribe({
              next: (res: HttpResponse<ITboReviewModel[]>) => this.initDataForMatTable(res, res.headers),
              error: (res: HttpErrorResponse) => this.onError(res.message),
            });
          return;
        } else {
