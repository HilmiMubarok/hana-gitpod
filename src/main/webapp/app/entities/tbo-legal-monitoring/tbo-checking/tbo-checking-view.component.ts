...
export class TboCheckingViewComponent implements OnInit {
  ...
  public creditProposal: ITboCheckingModel;
  public creditProposalStartState: ITboCheckingModel;
  ...
  public save(source: string): void {
    ...
    if (this.creditProposal.attributes.proposalType === null || this.creditProposal.attributes.proposalType === '') {
      ...
      if (this.creditProposal.id) {
        ...
        // } else
        if (this.router.url.split('/')[1] === 'tbo-legal-checking') {
          if (this.creditProposalOpinionHistoryComponent) {
  ...
  private preSave(status: string): ITboCheckingModel {
    for (let i = 0; i < this.tboCheckingService.partySliks.length; i++) {
      this.creditProposal.sliks = [...this.creditProposal.sliks, this.tboCheckingService.partySliks[i]];
    }
    const copyCreditProposal: ITboCheckingModel = lodash.cloneDeep(this.creditProposal);
