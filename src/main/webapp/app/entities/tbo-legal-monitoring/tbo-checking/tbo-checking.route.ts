...
import { ITboCheckingModel, TboCheckingModel } from './tbo-checking.model';
...
export class TboCheckingRoute implements Resolve<ITboCheckingModel> {
  constructor(private service: TboCheckingService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<ITboCheckingModel> | Observable<never> {
    const useTemplate = 'PERSON';
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((creditProposal: HttpResponse<ITboCheckingModel>) => {
    ...
    if (useTemplate) {
      return this.service.template(useTemplate).pipe(
        map((res: HttpResponse<ITboCheckingModel>) => res.body),
        mergeMap(res => {
          if (res) {
            return of(res);
          } else {
    ...
    const newItem = new TboCheckingModel();
