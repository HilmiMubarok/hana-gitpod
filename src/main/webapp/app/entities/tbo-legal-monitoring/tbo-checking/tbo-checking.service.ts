...
export class TboCheckingService extends AbstractEntityService<ITboCheckingModel> {
  ...
  private changgedColRelByCP?: ITboCheckingModel;
  private triggerChanggedColRelByCP = new BehaviorSubject<ITboCheckingModel>(this.changgedColRelByCP);
  public triggerChanggedColRelByCPObservable = this.triggerChanggedColRelByCP.asObservable();

  public changeColRelByCP(newCP: ITboCheckingModel) {
  ...
  public applicationGroubProduct(id: number): Observable<HttpResponse<any>> {
    return this.http
      .get<ITboCheckingModel[]>(MICROSERVICENAME.LOS + '/api/application-group-products/page/' + id, { observe: 'response' })
