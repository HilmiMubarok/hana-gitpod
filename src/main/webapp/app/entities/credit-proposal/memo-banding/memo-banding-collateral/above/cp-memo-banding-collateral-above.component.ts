...
export class CpMemoBandingCollateralAboveComponent implements OnChanges, OnInit, AfterViewInit {
  ...
  public countTotalLVBefore(): number {
    let data: ICollateralProperty;
    let result: number;
    result = 0;
    const collaterals: ICollateral[] = this.filtered;
    if (collaterals) {
  ...
  public slideChange($event) {
    if (this.isChecked === true) {
      ...
      if (
        ...
        for (const [index] of this.creditProposal.collateralProductRelations.entries()) {
          for (let j = 0; j < this.creditProposal.products.length; j++) {
