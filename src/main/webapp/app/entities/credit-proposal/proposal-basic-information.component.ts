...
export class ProposalBasicInformationComponent implements OnInit {
  ...
  ngOnInit() {
    this.creditProposalService.find(this.activatedRoute.snapshot.paramMap.get('id')).subscribe((res: HttpResponse<ICreditProposal>) => {
      ...
      const atributes = res.body.attributes;
      this.creditProposalList.attributes = {
        tabSummary: {
          strength: atributes.tabSummary === undefined ? '' : JSON.parse(atributes.tabSummary).strength,
          opportunities: atributes.tabSummary === undefined ? '' : JSON.parse(atributes.tabSummary).opportunities,
          weaknesses: atributes.tabSummary === undefined ? '' : JSON.parse(atributes.tabSummary).weaknesses,
          threats: atributes.tabSummary === undefined ? '' : JSON.parse(atributes.tabSummary).threats,
        },
      };
    });
  }
  ...
  save(): void {
    (this.tabSummary = {
      strength: this.creditProposalList.attributes.tabSummary.strength,
      opportunities: this.creditProposalList.attributes.tabSummary.opportunities,
      weaknesses: this.creditProposalList.attributes.tabSummary.weaknesses,
      threats: this.creditProposalList.attributes.tabSummary.threats,
    }),
      (this.creditProposalList.attributes.tabSummary = JSON.stringify(this.tabSummary));
