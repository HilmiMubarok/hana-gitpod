...
export class CovenantBackToBackGeneralHistoryComponent implements OnInit, OnDestroy {
  ...
  public loadCovenantBtbGeneral() {
    this.generalParameterService
      ...
      .pipe(
        ...
        tap(parsed => {
          const data = {
            finalCP: parsed['previousHistory']?.convenant?.standardDataGridBackToBackGeneral,
            previousDar: parsed['darRevHistory']?.convenant?.standardDataGridBackToBackGeneral,
            darRevision: this.standardDataGridBackToBackGeneral,
          };

          if (this.isOnCreditAgreement) {
            if (this.creditAgreement === 'FINAL CP') {
              this.standardDataGridBackToBackGeneral = data.finalCP;
            }
            if (this.creditAgreement === 'PREVIOUS DAR') {
              this.standardDataGridBackToBackGeneral = data.previousDar;
            }
            if (this.creditAgreement === 'DAR REVISION') {
              this.standardDataGridBackToBackGeneral = data.darRevision;
            }
          } else {
