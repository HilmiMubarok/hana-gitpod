...
export class CashCreditProposalService extends AbstractEntityService<ICreditProposal> {
  ...
  cashCreditProposalApproval(req?: any): Observable<HttpResponse<ICreditProposal[]>> {
    const options = createRequestOption(req);
    return this.http
      .get<ICreditProposal[]>(this.resourceUrl + '/cash-credit-proposals/cp-status-approval', { params: options, observe: 'response' })
      .pipe(map((res: HttpResponse<ICreditProposal[]>) => this.convertDateArrayFromServer(res)))
      .pipe(map((res: HttpResponse<ICreditProposal[]>) => this.preLoadItemArray(res)));
  }

  searchCP(req?: any): Observable<HttpResponse<ICreditProposal[]>> {
    const options = createRequestOption(req);
    return this.http
      .get<ICreditProposal[]>(this.resourceUrl + '/_search/cash-credit-proposals', { params: options, observe: 'response' })
      .pipe(map((res: HttpResponse<ICreditProposal[]>) => this.convertDateArrayFromServer(res)))
      .pipe(map((res: HttpResponse<ICreditProposal[]>) => this.preLoadItemArray(res)));
  }
