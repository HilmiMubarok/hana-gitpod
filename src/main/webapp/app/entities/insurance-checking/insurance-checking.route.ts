...
import { IInsuranceChecking, InsuranceChecking } from './insurance-checking.model';
...
import { InsuranceCheckingDetailComponent } from './insurance-checking-detail.component';

@Injectable({ providedIn: 'root' })
export class InsuranceCheckingResolve implements Resolve<IInsuranceChecking> {
  constructor(private service: InsuranceCheckingService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IInsuranceChecking> | Observable<never> {
    const useTemplate = 'PERSON';
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((creditProposal: HttpResponse<IInsuranceChecking>) => {
    ...
    if (useTemplate) {
      return this.service.template(useTemplate).pipe(
        map((res: HttpResponse<IInsuranceChecking>) => res.body),
        mergeMap(res => {
          if (res) {
            return of(res);
          } else {
    ...
    const newItem = new InsuranceChecking();
...
export const InsuranceCheckingRoute: Routes = [
  {
    path: '',
    component: InsuranceCheckingComponent,
    resolve: {
  ...
  {
    path: ':id/edit',
    component: InsuranceCheckingDetailComponent,
    resolve: {
      content: InsuranceCheckingResolve,
    },
    data: {
      authorities: ['ROLE_USER'],
      pageTitle: 'losgwApp.partyCif.home.title',
