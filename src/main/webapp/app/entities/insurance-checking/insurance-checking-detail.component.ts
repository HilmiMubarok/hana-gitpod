...
export class InsuranceCheckingDetailComponent implements OnInit {
  ...
  public creditProposal: IInsuranceChecking;
  public creditProposalStartState: IInsuranceChecking;
  ...
  setIsAllowSave(status: boolean) {
    ...
    if (this.creditProposal.id) {
      this.insuranceCheckingService.update(this.preSave(statusPreSave)).subscribe(res => {
  ...
  private preSave(status: string): IInsuranceChecking {
    for (let i = 0; i < this.insuranceCheckingService.partySliks.length; i++) {
      this.creditProposal.sliks = [...this.creditProposal.sliks, this.insuranceCheckingService.partySliks[i]];
    }
