...
export class InsuranceCheckingService extends AbstractEntityService<IInsuranceChecking> {
  ...
  private changgedColRelByCP?: IInsuranceChecking;
  private triggerChanggedColRelByCP = new BehaviorSubject<IInsuranceChecking>(this.changgedColRelByCP);
  public triggerChanggedColRelByCPObservable = this.triggerChanggedColRelByCP.asObservable();

  public changeColRelByCP(newCP: IInsuranceChecking) {
  ...
  public applicationGroubProduct(id: number): Observable<HttpResponse<any>> {
    return this.http
      .get<IInsuranceChecking[]>(MICROSERVICENAME.LOS + '/api/application-group-products/page/' + id, { observe: 'response' })
  ...
  protected convertDateArrayFromServer(res: HttpResponse<IInsuranceChecking[]>): HttpResponse<IInsuranceChecking[]> {
    res.body.forEach((creditProposal: IInsuranceChecking) => {
      //
