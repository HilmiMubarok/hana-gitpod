...
import { ILoanOPSChecking, LoanOPSChecking } from './loan-ops-checking.model';
...
export class LoanOpsCheckingResolve implements Resolve<ILoanOPSChecking> {
  constructor(private service: LoanOpsCheckingService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<ILoanOPSChecking> | Observable<never> {
    const useTemplate = 'PERSON';
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((creditProposal: HttpResponse<ILoanOPSChecking>) => {
          if (creditProposal.body) {
            ...
            if (!lodash.has(creditProposal.body.attributes, 'dataAssignToLoanOpsOfficer')) {
              ...
              creditProposal.body.attributes['dataAssignToLoanOpsOfficer'] = JSON.parse(
                creditProposal.body.attributes['dataAssignToLoanOpsOfficer']
              );
            }
    ...
    if (useTemplate) {
      return this.service.template(useTemplate).pipe(
        map((res: HttpResponse<ILoanOPSChecking>) => res.body),
        mergeMap(res => {
          if (res) {
            return of(res);
          } else {
    ...
    const newItem = new LoanOPSChecking();
...
export const LoanOpsCheckingRoute: Routes = [
  {
    path: '',
    component: LoanOpsCheckingComponent,
    resolve: {
  ...
  {
    path: ':id/edit',
    component: LoanOpsCheckingDetailComponent,
    resolve: {
      content: LoanOpsCheckingResolve,
    },
    data: {
      authorities: ['ROLE_USER'],
      pageTitle: 'losgwApp.partyCif.home.title',
