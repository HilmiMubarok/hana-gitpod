...
export class LoanOpsCheckingService extends AbstractEntityService<ILoanOPSChecking> {
  ...
  private changgedColRelByCP?: ILoanOPSChecking;
  private triggerChanggedColRelByCP = new BehaviorSubject<ILoanOPSChecking>(this.changgedColRelByCP);
  public triggerChanggedColRelByCPObservable = this.triggerChanggedColRelByCP.asObservable();

  public changeColRelByCP(newCP: ILoanOPSChecking) {
  ...
  public applicationGroubProduct(id: number): Observable<HttpResponse<any>> {
    return this.http
      .get<ILoanOPSChecking[]>(MICROSERVICENAME.LOS + '/api/application-group-products/page/' + id, { observe: 'response' })
  ...
  protected convertDateArrayFromServer(res: HttpResponse<ILoanOPSChecking[]>): HttpResponse<ILoanOPSChecking[]> {
    res.body.forEach((creditProposal: ILoanOPSChecking) => {
      //
