...
@Component({
  selector: 'jhi-document-checklist-opinion',
  template: `
    <div *ngFor="let item of dataArray">
      <mat-card *ngIf="item.statusId === 'ACTIVE'" style="margin-top: 12px">
        <mat-card-header>
          <mat-card-subtitle class="e-card-header-title">{{ item.description }}</mat-card-subtitle>
        </mat-card-header>
        <mat-card-content>
          <table mat-table [dataSource]="item.level" class="w-100">
            <ng-container matColumnDef="no">
              <th mat-header-cell *matHeaderCellDef width="5%" class="rounding-table-left">No</th>
              <td mat-cell *matCellDef="let i = index">{{ i + 1 }}</td>
            </ng-container>
            <ng-container matColumnDef="document">
              <th mat-header-cell *matHeaderCellDef width="40%">Document Name</th>
            ...
            <ng-container matColumnDef="dueDate">
              ...
              <td mat-cell *matCellDef="let element">
                <span *ngIf="element.dueDate !== 'null'">{{ element.dueDate | date: 'YYYY-MM-dd' }}</span>
              </td>
            </ng-container>
            ...
            <ng-container matColumnDef="action">
              <th mat-header-cell *matHeaderCellDef width="10%" class="rounding-table-right">Action</th>
              <td mat-cell *matCellDef="let element">
                <button mat-icon-button (click)="openDialog(element, 'add', item.description)">
                  <mat-icon>create</mat-icon>
                </button>

                <button mat-icon-button (click)="openDialog(element, 'view', item.description)">
                  <mat-icon>remove_red_eye</mat-icon>
                </button>
              </td>
            </ng-container>
            <tr class="mat-row" *matNoDataRow>
              <td class="mat-cell" [attr.colspan]="['no', 'document', 'category', 'remarks', 'dueDate', 'status', 'action'].length">
                No records found.
              </td>
            </tr>
            <tr mat-header-row *matHeaderRowDef="['no', 'document', 'category', 'remarks', 'dueDate', 'status', 'action']"></tr>
            <tr mat-row *matRowDef="let row; columns: ['no', 'document', 'category', 'remarks', 'dueDate', 'status', 'action']"></tr>
          </table>
        </mat-card-content>
      </mat-card>
    </div>
...
export class DocumentChecklistOpinionComponent implements OnInit {
  public files: any[];
                ...
                this.documentTypeService.documentTypeList('DOC_LA').subscribe((res3: any) => {
                  ...
                  for (let i = 0; i < result.length; i++) {
                    this.documentTypeService.documentTypeList(result[i].id).subscribe((re: any) => {
                      ...
                      this.dataArray = result.filter(data => data.id === 'DOC_LA_OPINION');
                    });
                  }
                });
              });
  ...
  public donwload() {
    this.getBucket().then(() => {
      this.getFiles(String(this.creditProposal.id)).then(() => {
        ...
        const downloadPromises = this.fileUrl.map(async file => {
          try {
