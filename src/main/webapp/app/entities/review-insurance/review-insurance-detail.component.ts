...
import {
  ...
  BASIC_SUBMENU_REVIEW_INSURANCE_MEMO,
...
@Component({
  ...
  templateUrl: './review-insurance-floating.component.html',
  styleUrls: ['./review-insurance.css'],
})
export class ReviewInsuranceDetailComponent implements OnInit {
  ...
  public creditProposal: IReviewInsurance;
  public creditProposalStartState: IReviewInsurance;
  ...
  constructor(
    ...
    private reviewInsuranceProcessService: ReviewInsuranceProcessService,
    ...
    this.subMenu = BASIC_SUBMENU_REVIEW_INSURANCE_MEMO;
  ...
  setIsAllowSave(status: boolean) {
    ...
    if (this.creditProposal.id) {
      this.reviewInsuranceService.update(this.preSave(statusPreSave)).subscribe(res => {
  ...
  private getTasks(): void {
    ...
    this.reviewInsuranceProcessService
  ...
  private preSave(status: string): IReviewInsurance {
    for (let i = 0; i < this.reviewInsuranceService.partySliks.length; i++) {
      this.creditProposal.sliks = [...this.creditProposal.sliks, this.reviewInsuranceService.partySliks[i]];
    }
