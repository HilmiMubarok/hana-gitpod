...
export class ReviewInsuranceService extends AbstractEntityService<IReviewInsurance> {
  ...
  private changgedColRelByCP?: IReviewInsurance;
  private triggerChanggedColRelByCP = new BehaviorSubject<IReviewInsurance>(this.changgedColRelByCP);
  public triggerChanggedColRelByCPObservable = this.triggerChanggedColRelByCP.asObservable();

  public changeColRelByCP(newCP: IReviewInsurance) {
  ...
  public applicationGroubProduct(id: number): Observable<HttpResponse<any>> {
    return this.http
      .get<IReviewInsurance[]>(MICROSERVICENAME.LOS + '/api/application-group-products/page/' + id, { observe: 'response' })
  ...
  protected convertDateArrayFromServer(res: HttpResponse<IReviewInsurance[]>): HttpResponse<IReviewInsurance[]> {
    res.body.forEach((creditProposal: IReviewInsurance) => {
      //
