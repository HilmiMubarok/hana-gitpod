...
import { IReviewInsurance, ReviewInsurance } from './review-insurance.model';
...
import { ReviewInsuranceDetailComponent } from './review-insurance-detail.component';

@Injectable({ providedIn: 'root' })
export class ReviewInsuranceReviewResolve implements Resolve<IReviewInsurance> {
  constructor(private service: ReviewInsuranceService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IReviewInsurance> | Observable<never> {
    const useTemplate = 'PERSON';
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((creditProposal: HttpResponse<IReviewInsurance>) => {
    ...
    if (useTemplate) {
      return this.service.template(useTemplate).pipe(
        map((res: HttpResponse<IReviewInsurance>) => res.body),
        mergeMap(res => {
          if (res) {
            return of(res);
          } else {
    ...
    const newItem = new ReviewInsurance();
...
export const ReviewInsuranceReviewRoute: Routes = [
  {
    path: '',
    component: ReviewInsuranceComponent,
    resolve: {
  ...
  {
    path: ':id/edit',
    component: ReviewInsuranceDetailComponent,
    resolve: {
      content: ReviewInsuranceReviewResolve,
    },
    data: {
      authorities: ['ROLE_USER'],
      pageTitle: 'losgwApp.partyCif.home.title',
