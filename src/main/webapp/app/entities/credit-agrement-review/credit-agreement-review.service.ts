...
export class CreditAgreementReviewService extends AbstractEntityService<ICreditAgreementReview> {
  ...
  private changgedColRelByCP?: ICreditAgreementReview;
  private triggerChanggedColRelByCP = new BehaviorSubject<ICreditAgreementReview>(this.changgedColRelByCP);
  public triggerChanggedColRelByCPObservable = this.triggerChanggedColRelByCP.asObservable();

  public changeColRelByCP(newCP: ICreditAgreementReview) {
  ...
  public applicationGroubProduct(id: number): Observable<HttpResponse<any>> {
    return this.http
      .get<ICreditAgreementReview[]>(MICROSERVICENAME.LOS + '/api/application-group-products/page/' + id, { observe: 'response' })
  ...
  protected convertDateArrayFromServer(res: HttpResponse<ICreditAgreementReview[]>): HttpResponse<ICreditAgreementReview[]> {
    res.body.forEach((creditProposal: ICreditAgreementReview) => {
      //
