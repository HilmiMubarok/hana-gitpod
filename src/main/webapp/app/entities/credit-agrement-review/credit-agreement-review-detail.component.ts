...
@Component({
  ...
  templateUrl: './credit-agreement-review-floating.component.html',
...
export class CreditAgreementReviewDetailComponent implements OnInit {
  ...
  public creditProposalStartState: ICreditAgreementReview;
  ...
  setIsAllowSave(status: boolean) {
    ...
    if (this.creditProposal.id) {
      this.creditAgreementReviewService.update(this.preSave(statusPreSave)).subscribe(res => {
  ...
  public menuCreditProposal() {
    if (this.parentPath === 'cp-status-approval') {
      if (this.creditProposal.attributes.proposalType === ID_GREATER_15_BN && this.creditProposal.attributes.proposalType !== undefined) {
        ...
        if (this.creditProposal.attributes['previousOfferingLetter']) {
          this.subMenu = [...PROPOSAL_TYPE, { id: 'memo-banding', text: 'Memo Banding' }];
        } else {
          this.subMenu = PROPOSAL_TYPE;
        }
      }
  ...
  private preSave(status: string): ICreditAgreement {
    for (let i = 0; i < this.creditAgreementReviewService.partySliks.length; i++) {
      this.creditProposal.sliks = [...this.creditProposal.sliks, this.creditAgreementReviewService.partySliks[i]];
    }
