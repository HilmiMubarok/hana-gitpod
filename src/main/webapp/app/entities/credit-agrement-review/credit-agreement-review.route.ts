...
import { CreditAgreementReview, ICreditAgreementReview } from './credit-agreement-review.model';

@Injectable({ providedIn: 'root' })
export class CreditAgreementReviewResolve implements Resolve<ICreditAgreementReview> {
  constructor(private service: CreditAgreementReviewService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<ICreditAgreementReview> | Observable<never> {
    const useTemplate = 'default';
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((creditAgreement: HttpResponse<ICreditAgreementReview>) => {
    ...
    if (useTemplate) {
      return this.service.template(useTemplate).pipe(
        map((res: HttpResponse<ICreditAgreementReview>) => res.body),
        mergeMap(res => {
          if (res) {
            return of(res);
          } else {
    ...
    const newItem = new CreditAgreementReview();
...
export const creditAgrementReviewRoute: Routes = [
  {
    path: '',
    component: CreditAgreementReviewComponent,
    resolve: {
  ...
  {
    path: ':id/edit',
    component: CreditAgreementReviewDetailComponent,
    resolve: {
