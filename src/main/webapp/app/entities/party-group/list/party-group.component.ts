...
export class PartyGroupComponent implements OnInit {
  ...
  search(query: string): void {
    if (query && ['groupName', 'prefix', 'officePhone', 'otherPhone', 'officeMail', 'faxOffice', 'taxIdNumber'].includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  protected onSuccess(data: IPartyGroup[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/party-group'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
