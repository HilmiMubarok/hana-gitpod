...
export class DarRevisionService extends AbstractEntityService<IDarRevisionModel> {
  ...
  private changgedColRelByCP?: IDarRevisionModel;
  private triggerChanggedColRelByCP = new BehaviorSubject<IDarRevisionModel>(this.changgedColRelByCP);
  public triggerChanggedColRelByCPObservable = this.triggerChanggedColRelByCP.asObservable();

  public changeColRelByCP(newCP: IDarRevisionModel) {
  ...
  public applicationGroubProduct(id: number): Observable<HttpResponse<any>> {
    return this.http
      .get<IDarRevisionModel[]>(MICROSERVICENAME.LOS + '/api/application-group-products/page/' + id, { observe: 'response' })
