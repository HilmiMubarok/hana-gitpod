...
@Component({
  selector: 'jhi-credit-facility-view',
  templateUrl: './credit-facility-view.component.html',
})
export class CreditFacilityViewComponent extends AbstractEntityBaseViewComponent<ICreditFacility> implements OnChanges {
  @Input() id: number;
  ...
  constructor(
    ...
    super(creditFacilityService, messageService, elementRef, dataUtils, account, eventManager);
    this.item = new CreditFacility();
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['id']) {
      ...
      if (this.id) {
        this.item = new CreditFacility();
        this.creditFacilityService.find(this.id).subscribe(result => {
  ...
  set creditFacility(creditFacility: ICreditFacility) {
    this.item = creditFacility;
  }
